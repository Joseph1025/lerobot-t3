#!/usr/bin/env python
"""
Convert HDF5 robot data to LeRobot dataset format (Three Cameras: High + Both Wrists).

This script converts HDF5 files containing robot demonstrations to the LeRobot dataset format,
including the high camera view and both wrist cameras (left and right), but excluding any other cameras.

Usage:
    python convert_hdf5_to_lerobot_three_cams.py --input_dir /path/to/hdf5/files --output_dir /path/to/output --fps 30
"""
...
def create_features_dict() -> Dict[str, Dict[str, Any]]:
    ...
    Includes high camera and both wrist cameras.
    ...
    features = {
        ...
        # Three camera images: high camera and both wrist cameras
        "observation.images.cam_high": {"dtype": "video", "shape": (480, 640, 3), "names": ["height", "width", "channels"]},
        "observation.images.cam_left_wrist": {"dtype": "video", "shape": (480, 640, 3), "names": ["height", "width", "channels"]},
        "observation.images.cam_right_wrist": {"dtype": "video", "shape": (480, 640, 3), "names": ["height", "width", "channels"]},
...
def validate_hdf5_structure(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        with h5py.File(file_path, 'r') as f:
            ...
            # Check images structure - require all three cameras
            ...
            # Validate image shape for all cameras
...
def load_hdf5_episode(file_path: Path) -> Dict[str, np.ndarray]:
    ...
    Loads high camera and both wrist cameras.
...
def convert_hdf5_to_lerobot_three_cams(
    input_dir: Path,
    output_dir: Path,
    fps: int = 30,
    repo_id: str = "my_robot_dataset_three_cams",
    ...
    """
    Convert HDF5 files to LeRobot dataset format (Three Cameras: High + Both Wrists).
    ...
    logger.info("Creating LeRobot dataset (Three Cameras: High + Both Wrists)...")
    dataset = LeRobotDataset.create(
        ...
        image_writer_threads=12,  # 4 threads per camera (3 cameras = 12 threads)
    )
    
    # Log dataset configuration
    logger.info(f"Dataset video keys: {dataset.meta.video_keys}")
    logger.info(f"Dataset camera keys: {dataset.meta.camera_keys}")
    ...
    for episode_idx, hdf5_file in enumerate(hdf5_files):
        ...
        for frame_idx in range(episode_length):
            ...
            # Include all three camera views
            frame = {
    ...
    for video_file in video_files:
        logger.info(f"  - {video_file.relative_to(output_dir)}")
    ...
    print(f"Camera views: High camera + Left wrist + Right wrist")
    ...
    if push_to_hub:
        ...
        try:
            dataset.push_to_hub(
                tags=tags or ["lerobot", "robotics", "demonstration", "three_cameras"],
                license=license,
                private=private
            )
...
def main():
    parser = argparse.ArgumentParser(description="Convert HDF5 robot data to LeRobot dataset format (Three Cameras: High + Both Wrists)")
    ...
    parser.add_argument("--repo_id", type=str, default="my_robot_dataset_three_cams", help="Repository ID for dataset")
    ...
    # Convert dataset
    convert_hdf5_to_lerobot_three_cams(
