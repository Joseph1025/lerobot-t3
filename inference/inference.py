...
def get_model_config(args):
    ...
    if args.policy_class == 'ACT':
        policy_config = {'lr': args.lr,
                         ...
                         # T3 configuration
                         't3_config_path': getattr(args, 't3_config_path', None),
                         't3_encoder_path': getattr(args, 't3_encoder_path', None)
                         }
...
def inference_process(args, config, ros_operator, policy, stats, t, pre_action):
    ...
    while True and not rospy.is_shutdown():
        ...
        if not result:
            ...
            continue
        print_flag = True
        (img_left, img_right, img_front, puppet_arm_left, puppet_arm_right) = result
...
def model_inference(args, config, ros_operator, save_episode=True):
    ...
    policy = make_policy(config['policy_class'], config['policy_config'],config['pretrain_ckpt_dir'])
...
class RosOperator:
    ...
    def puppet_arm_publish_continuous(self, left, right):
        ...
        while True and not rospy.is_shutdown():
            # print(f"left_arm: {left_arm}, right_arm: {right_arm}") yes value exist
            # print(f"puppet arm left: {self.puppet_arm_left_deque}") empty
            # print(f"puppet arm right: {self.puppet_arm_right_deque}") empty
    ...
    def get_frame(self):
        # Wait for queues to be non-empty
        if len(self.img_left_deque) == 0 or len(self.img_right_deque) == 0 or len(self.img_front_deque) == 0 or \
                len(self.puppet_arm_left_deque) == 0 or len(self.puppet_arm_right_deque) == 0:
            time.sleep(2)

        # print("len of img_left_deque", len(self.img_left_deque))
        ...
        return (img_left, img_right, img_front, puppet_arm_left, puppet_arm_right)
    ...
    def init_ros(self):
        print("init_ros started")
        self.img_left_topic ="/camera1/color/image_raw"
        self.img_right_topic ="/camera2/color/image_raw"
        self.img_front_topic ="/camera3/color/image_raw"
        self.puppet_arm_left_topic ="/puppet/joint_left"
        self.puppet_arm_right_topic ="/puppet/joint_right"
        self.puppet_arm_left_cmd_topic ="/left_joint_states"
        self.puppet_arm_right_cmd_topic ="/right_joint_states"

        # Only initialize ROS node if it hasn't been initialized
        try:
            rospy.init_node('joint_state_publisher', anonymous=True)
        except rospy.exceptions.ROSException:
            # Node already initialized, that's fine
            pass
...
def get_arguments():
    ...
    parser.add_argument('--batch_size', action='store', type=int, help='batch_size', default=1, required=False)
    ...
    parser.add_argument('--puppet_arm_left_cmd_topic', action='store', type=str, help='puppet_arm_left_cmd_topic',
                        default='/left_joint_states', required=False)
    parser.add_argument('--puppet_arm_right_cmd_topic', action='store', type=str, help='puppet_arm_right_cmd_topic',
                        default='/right_joint_states', required=False)
    ...
    # parser.add_argument('--arm_steps_length', action='store', type=float, help='arm_steps_length',
    #                     default=[0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.4], required=False)
